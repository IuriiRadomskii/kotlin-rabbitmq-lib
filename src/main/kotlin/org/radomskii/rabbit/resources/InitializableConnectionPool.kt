...
internal class InitializableConnectionPool(
    ...
    private val connections = CopyOnWriteArrayList<ManagedConnectionImpl>()
    ...
    override fun nextConnection(): ManagedConnection {
    ...
    private fun scheduleConnectionAttempt(
        numberOfAttempt: Int,
        onSuccess: (ManagedConnectionImpl) -> Unit,
        onDone: () -> Unit = {}
    ) {
        ...
        connectionRetryScheduler.execute {
            ...
            try {
                val connection = ManagedConnectionImpl(connectionFactory.newConnection())
                if (closed.get()) {
                    connection.close()
                } else {
    ...
    private fun isNearChannelCapacity(connection: ManagedConnectionImpl): Boolean {
    ...
    private companion object {
        val log = LoggerFactory.getLogger(InitializableConnectionPool::class.java)
